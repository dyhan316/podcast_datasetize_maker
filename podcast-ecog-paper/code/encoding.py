...
def main(
    ...
    if band == "highgamma":
        ...
        print("NON BANDPASSED DATA")
        # Match only files without a desc (i.e., non-highgamma)
        edf_path = BIDSPath(
            root=bids_root, datatype="ieeg", description=None, suffix="ieeg", extension=".fif"
        )
        edf_paths = edf_path.match()
        edf_paths = [p for p in edf_paths if "highgamma" not in p.basename]
    ...
    for edf_path in edf_paths:
        ...
        #! raw.get_data().shape = (99, 921600) wit 70Hz highpassed (gamma!) => NOT RAW! 
        #! since sfreq 512, that's 99 channels and 1800 seconds recording
        ...
        events[:, 0] = (df.start * sfreq).astype(int) #*events.shape = (5136, 3)
        epochs = mne.Epochs(
            raw,
            events,
            tmin=tmin,
        ...
        import pdb; pdb.set_trace() #* epochs.get_data().shape
        epochs = epochs.resample(
            sfreq=32, npad="auto", method="fft", window="hamming", n_jobs=16  # NOTE
        )
        #! modify from 16 jobs to smaller later!
        #* epochs.get_data(copy=False).shape = (5130, 99, 128)) #* 5130 : numbe rof labels
        #! THIS SHOULD BE CHANGED : (1) 128 resampling not useful for decoding (not encoding) 
        #* also 128 => 4s (in 32Hz) => becuase we tmin=-2, tmax=2 (i.e. 4s window)
        import pdb ; pdb.set_trace()

        # Prepare data for modeling
        epochs_data = epochs.get_data(copy=True)
        epochs_data = epochs_data.reshape(len(epochs), -1) #! I.E. FLATTEN THE CHANNELS with TIMEPOINTS
